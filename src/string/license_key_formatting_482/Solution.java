package string.license_key_formatting_482;
...
public class Solution {
    String licenseKeyFormatting(String licence, int length) {
        StringBuilder sb = new StringBuilder();
        int cnt = length;
        for (int i= licence.length()-1; i >= 0; i--) {
            char curr = licence.charAt(i);
            if (curr == '-') continue;
            if (cnt == 0) {
                sb.insert(0, '-');
                cnt = length;
